...
def merge_consecutive_intervals(
    ...
    for entry in comparison_data[1:]:
        if entry.get("differences", {}).get("executor_count_diff") == current.get(
            "differences", {}
        ).get("executor_count_diff"):
            # Extend current time range to this entry's end
            current["timestamp_range"] = (
                f"{current['timestamp_range'].split(' to ')[0]} to {entry['timestamp_range'].split(' to ')[1]}"
            )
        else:
...
def merge_intervals(
    items: List[Dict[str, Any]], same_key: str = "executor_count_diff"
) -> List[Dict[str, Any]]:
    ...
    for entry in items[1:]:
        if entry.get("differences", {}).get(same_key) == current.get(
            "differences", {}
        ).get(same_key):
            current["timestamp_range"] = (
                f"{current['timestamp_range'].split(' to ')[0]} to {entry['timestamp_range'].split(' to ')[1]}"
        ...
        timeline.append(
            {
                ...
                "stage_duration_hours": (stage_end - stage_start).total_seconds()
                / 3600,
...
def build_app_executor_timeline(
    ...
    end_time = app.attempts[0].end_time or (
        start_time and start_time + timedelta(hours=24)
    )
    if not start_time:
        return None
    ...
    return {
        ...
        "summary": {
            ...
            "peak_executor_count": max(
                (t["active_executor_count"] for t in timeline), default=0
            ),
            "avg_executor_count": (
                sum(t["active_executor_count"] for t in timeline) / len(timeline)
            )
            if timeline
            else 0,
