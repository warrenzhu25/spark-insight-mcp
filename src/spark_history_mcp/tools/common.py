...
def resolve_legacy_tool(name: str, fallback: F) -> F:
    ...
    Many unit tests patch ``spark_history_mcp.tools`` functions directly.
    ...
    tools_module = sys.modules.get("spark_history_mcp.tools")
    if tools_module:
...
def get_client_or_default(ctx: Optional[Any] = None, server_name: Optional[str] = None):
    """Resolve a Spark client from MCP context and optional server name."""
    if ctx is None:
