...
class TestCompareSessionManagement:
    ...
    def test_save_and_load_comparison_context(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "spark_history_mcp.cli.commands.compare.get_session_file",
...
class TestAppIdentifierResolution:
    ...
    def test_resolve_app_identifiers_single_name(self, mock_resolve):
        ...
        mock_apps = [
            MagicMock(
                id="app-123",
                name="ETL Pipeline",
                attempts=[MagicMock(start_time="2023-01-01")],
            ),
            MagicMock(
                id="app-456",
                name="ETL Pipeline",
                attempts=[MagicMock(start_time="2023-01-02")],
            ),
        ]
        mock_resolve.return_value = ("app-123", "app-456", mock_apps)

        app_id1, app_id2, feedback = resolve_app_identifiers(
            "ETL Pipeline", None, mock_client
        )
    ...
    def test_resolve_app_identifiers_mixed(self, mock_resolve):
        ...
        app_id1, app_id2, feedback = resolve_app_identifiers(
            "app-20231201-aaaaaa", "Daily Job", mock_client
        )
...
class TestGetAppContext:
    ...
    def test_get_app_context_with_both_ids(self):
        ...
        with patch(
            "spark_history_mcp.cli.commands.compare.save_comparison_context"
        ) as mock_save:
...
class TestCompareAppsCommand:
    ...
    def test_compare_apps_basic_success(
        self,
        mock_compare_perf,
        mock_save_context,
        mock_resolve,
        ...
        result = cli_runner.invoke(
            compare,
            ["apps", "app-123", "app-456", "--format", "json", "--threshold", "0.15"],
    ...
    def test_compare_apps_invalid_threshold(self, cli_runner):
        ...
        result = cli_runner.invoke(
            compare,
            ["apps", "app-123", "app-456", "--threshold", "invalid"],
            obj={"config_path": CONFIG_PATH},
        )
    ...
    def test_compare_apps_missing_arguments(self, cli_runner):
        ...
        result = cli_runner.invoke(compare, ["apps"], obj={"config_path": CONFIG_PATH})
...
class TestCompareStagesCommand:
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch("spark_history_mcp.tools.compare_stages")
    def test_compare_stages_with_context(
        self, mock_compare_stages, mock_get_client, mock_get_context, cli_runner
    ):
        ...
        result = cli_runner.invoke(
            compare,
            ["stages", "1", "2", "--significance-threshold", "0.2", "--format", "json"],
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch("spark_history_mcp.tools.compare_stages")
    def test_compare_stages_with_apps_override(
        self, mock_compare_stages, mock_get_client, cli_runner
    ):
        ...
        result = cli_runner.invoke(
            compare,
            ["stages", "1", "2", "--apps", "app-789", "app-012"],
            obj={"config_path": CONFIG_PATH},
        )
...
class TestCompareTimelineCommand:
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_app_context")
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch("spark_history_mcp.tools.compare_app_executor_timeline")
    def test_compare_timeline_basic(
        self, mock_compare_timeline, mock_get_client, mock_get_context, cli_runner
    ):
        ...
        mock_compare_timeline.assert_called_once_with(
            app_id1="app-123", app_id2="app-456", server="local", interval_minutes=5
        )
...
class TestCompareExecutorsCommand:
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_app_context")
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch("spark_history_mcp.tools.compare_app_executors")
    def test_compare_executors_with_filtering(
        self, mock_compare_execs, mock_get_client, mock_get_context, cli_runner
    ):
...
class TestCompareStatusAndClearCommands:
    ...
    def test_clear_command(self, mock_clear, mock_load, cli_runner):
        ...
        result = cli_runner.invoke(compare, ["clear"], obj={"config_path": CONFIG_PATH})
...
class TestAdditionalCompareCommands:
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_app_context")
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch("spark_history_mcp.tools.compare_app_resources")
    def test_compare_resources_with_override(
        self, mock_compare_resources, mock_get_client, mock_get_context, cli_runner
    ):
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_app_context")
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch("spark_history_mcp.tools.compare_stage_executor_timeline")
    def test_stage_timeline_basic(
        self, mock_stage_timeline, mock_get_client, mock_get_context, cli_runner
    ):
    ...
    def test_jobs_basic(
        self, mock_compare_jobs, mock_get_client, mock_resolve, cli_runner
    ):
        mock_get_client.return_value = MagicMock()
        ...
        result = cli_runner.invoke(
            compare,
            ["jobs", "--apps", "app-1", "app-2", "--format", "json"],
            obj={"config_path": CONFIG_PATH},
        )
        assert result.exit_code == 0
    ...
    def test_apps_interactive_quit(
        self, mock_getchar, mock_compare_perf, mock_get_client, mock_resolve, cli_runner
    ):
        mock_get_client.return_value = MagicMock()
        ...
        result = cli_runner.invoke(
            compare,
            ["apps", "app-1", "app-2", "--interactive", "--format", "json"],
            obj={"config_path": CONFIG_PATH},
        )
        assert result.exit_code == 0
        mock_compare_perf.assert_called_once()
...
class TestCompareMenus:
    ...
    def test_show_post_stage_menu_stage_timeline(
        self, mock_getchar, mock_exec_stage_timeline
    ):
        from spark_history_mcp.cli.commands.compare import show_post_stage_menu
...
class TestCompareStagesPostMenu:
    ...
    @patch(
        ...
        return_value=("app-1", "app-2", "local"),
    )
    @patch("spark_history_mcp.tools.compare_stages")
    @patch("click.getchar", return_value="q")
...
class TestCompareMoreErrorsAndBranches:
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch("spark_history_mcp.tools.compare_app_jobs", side_effect=Exception("boom"))
    def test_jobs_error_handling(
        self, mock_comp, mock_get_client, mock_get_ctx, cli_runner
    ):
        mock_get_client.return_value = MagicMock()
        result = cli_runner.invoke(compare, ["jobs"], obj={"config_path": CONFIG_PATH})
        assert result.exit_code != 0
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch(
        "spark_history_mcp.tools.compare_app_resources", side_effect=Exception("boom")
    )
    def test_resources_error_handling(
        self, mock_comp, mock_get_client, mock_get_ctx, cli_runner
    ):
        mock_get_client.return_value = MagicMock()
        result = cli_runner.invoke(
            compare, ["resources"], obj={"config_path": CONFIG_PATH}
        )
        assert result.exit_code != 0
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch(
        "spark_history_mcp.tools.compare_app_executor_timeline",
    ...
    def test_timeline_error_handling(
        ...
        mock_get_client.return_value = MagicMock()
        result = cli_runner.invoke(
            compare, ["timeline"], obj={"config_path": CONFIG_PATH}
        )
        assert result.exit_code != 0
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch(
        "spark_history_mcp.tools.compare_stage_executor_timeline",
        side_effect=Exception("boom"),
    )
    def test_stage_timeline_error_handling(
        self, mock_comp, mock_get_client, mock_get_ctx, cli_runner
    ):
        mock_get_client.return_value = MagicMock()
        result = cli_runner.invoke(
    ...
    @patch("spark_history_mcp.cli.commands.compare.get_spark_client")
    @patch(
        "spark_history_mcp.tools.compare_app_executors", side_effect=Exception("boom")
    )
    def test_executors_error_handling(
        self, mock_comp, mock_get_client, mock_get_ctx, cli_runner
    ):
        mock_get_client.return_value = MagicMock()
        result = cli_runner.invoke(
            compare, ["executors"], obj={"config_path": CONFIG_PATH}
        )
        assert result.exit_code != 0
        ...
        data = {
            "performance_comparison": {
                "stages": {
                    "top_stage_differences": [
                        {
                            "stage_name": "Very Long Stage Name That Should Be Truncated For Display Purposes",
                            "time_difference": {
                                "percentage": 20,
...
class TestPerformanceAlias:
    ...
    def test_performance_alias_invocation(self, cli_runner):
        ...
        result = cli_runner.invoke(
            compare,
            ["performance", "app-1", "app-2", "--format", "json"],
            obj={"config_path": CONFIG_PATH},
        )
        assert result.exit_code != 0
...
class TestCompareCommandParameterValidation:
    ...
    def test_invalid_format_option(self, cli_runner):
        ...
        result = cli_runner.invoke(
            compare,
            ["apps", "app-123", "app-456", "--format", "invalid"],
            obj={"config_path": CONFIG_PATH},
        )
