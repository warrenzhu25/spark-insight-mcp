...
def test_compare_stage_executor_timeline_basic(mock_get_context):
    ...
    ex = SimpleNamespace(
        ...
        add_time=now,
        remove_time=now + timedelta(minutes=1),
    )
...
def test_compare_app_executor_timeline_basic(mock_get_context):
    ...
    ex = SimpleNamespace(
        ...
        add_time=now,
        remove_time=now + timedelta(minutes=2),
    )
    c.list_all_executors.side_effect = [[ex], [ex]]
    st = SimpleNamespace(
        stage_id=1,
...
def test_get_app_summary_basic(mock_get_context):
    ...
    attempt = SimpleNamespace(
        start_time=now, end_time=now + timedelta(minutes=5), duration=5 * 60 * 1000
    )
    ...
    st1 = SimpleNamespace(
        status="COMPLETE",
        executor_run_time=60_000,
        executor_cpu_time=30_000_000_000,
        jvm_gc_time=5_000,
        input_bytes=10_000_000,
        output_bytes=5_000_000,
        shuffle_read_bytes=2_000_000,
        shuffle_write_bytes=1_000_000,
    ...
    ex1 = SimpleNamespace(
        add_time=now,
        remove_time=now + timedelta(minutes=4),
        total_cores=2,
        max_memory=512 * 1024 * 1024,
    )
...
def test_compare_app_summaries_basic(mock_get_context, mock_get_app_summary):
    ...
    s1 = {
        ...
        "total_executor_runtime_minutes": 5.0,
    ...
    s2 = {
        ...
        "application_name": "A2",
        "analysis_timestamp": "t",
        "application_duration_minutes": 20.0,
        "total_executor_runtime_minutes": 4.0,
...
def test_compare_stages_basic(mock_get_context):
    ...
    st1 = SimpleNamespace(
        name="S1",
        status="COMPLETE",
        completion_time=now + timedelta(seconds=20),
        first_task_launched_time=now,
        task_metrics_distributions=None,
...
def test_get_stage_task_summary(mock_get_context):
    ...
    assert (
        kwargs["app_id"] == "app"
        and kwargs["stage_id"] == 7
        and kwargs["attempt_id"] == 2
    )
